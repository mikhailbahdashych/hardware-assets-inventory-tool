...
- **Sign-ins are a shared budget.** The login rate limit is 10 per 15 minutes per address, the suite is one address, and a full run currently spends **9**: `auth.spec` 3 (a wrong password and two real sign-ins), the session helper's first sign-in 1, `two-factor.spec` 2, `roles.spec` 1, `x-change-password.spec` 2. One more sign-in fits; past that, free one first (replace a `signInFresh` with `signIn`, or pin the behaviour in an API test instead) — the eleventh login answers 429 and reads as a broken login page, not as a budget.
