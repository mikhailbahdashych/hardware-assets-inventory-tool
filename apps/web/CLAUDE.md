...
- **The command palette does the keyboard work its own footer promises.** It says "↑↓ navigate · ↵ open · esc close", so it does all three. `components/app/palette.ts` is the pure half — grouping the rows and filtering the **commands** by permission — so the list can be tested without a keyboard; `CommandPalette.tsx` is one flat roving index over it, wrapping at both ends, with the highlight returning to the top as the results change under it. Assets and people come from `useSearch(q)` (`GET /search`), debounced, already searched and already capped at four a group: the two whole-list caches it used to read are pages now, and a palette that searched only the page you happened to be on would find less than the app knows. An empty query still shows a few of each, because the endpoint answers one.
- **Anything that floats over a table is portalled to the body.** `Menu` (the design's overflow trigger — an IconButton-style bordered square with the `dots` icon, because the original bare `···` glyph vanished beside a column of em dashes) and `Dropdown` position themselves from the trigger's own rect for a reason: a `DataTable` cell clips its overflow — that clip is what gives the other cells their ellipsis — and the card around it clips to its border radius. A popover rendered in place is a popover the row eats, and no z-index fixes it.
...
- `components/ui/` — primitives (Button, Pill, DataTable, Modal, …), one component + one CSS module + one `types/` module each, exported from `index.ts`. Interactive primitives have behavior tests in `primitives.test.tsx`. The one CSS module with no component beside it is `FormModal.module.css`, shared by the thirteen feature modals so their fields, hints and footers line up — it is a stylesheet, not a missing component.
- `components/app/` — shell chrome: `AppShell`, `Sidebar`, `Topbar`, `PageContainer`, `nav.ts` (pure section/breadcrumb logic), `useThemeControls.ts`. **The sidebar is two named `<nav>` landmarks** — "Inventory" on top, "Workspace" (Members, Activity log, Workflow, Custom fields, Roles, API tokens, Admin) pinned to the bottom — an owner-sanctioned departure from the handoff's single nav with a 10px gap: managing the workspace sits next to the person doing it. A bare `getByRole('navigation')` is ambiguous now; queries name the landmark (absence checks stay page-wide, deliberately).
- `features/<area>/` — pages and feature modals per area (auth, dashboard, assets, employees, members, admin, api-tokens, import, workflow, roles, custom-fields, notifications). `members/CopyLinkModal.tsx` is shared with the employee form, which can invite the person it just created, and with the API token modal, which ends on a value shown exactly once for the same reason; showing a one-time secret is one screen, so they borrow it rather than growing three. `admin/` holds **two pages, not one with tabs**: `ActivityLogPage` (`/activity`) is for reading what happened, `AdminPage` (`/admin`) for changing how the workspace behaves. Both are admin-only, and their old tab URLs (`/admin/activity`, `/admin/settings`) still redirect — `/admin/activity?type=auth` was a shareable link, so `LegacyActivityRedirect` carries the query string across.
- `providers/` — ThemeProvider, ToastProvider, ModalProvider. Hooks live next to their provider.
  **ModalProvider owns the seven app-level modals** (palette, new asset, add employee, invite member, import, widgets, change password) and `components/app/ModalHost.tsx` renders whichever is open, mounted once in the shell. They live there because the command palette opens four of them from anywhere and two are reachable from more than one screen; without an owner they would be the same boolean declared three times. Anything carrying a **subject** — assign, check in, change status, edit — stays local state on the page that knows the subject.
...
- Reads go through hooks in `api/queries.ts`; add the key to `queryKeys` first. Writes live in `api/mutations.ts` and go through one of the two coarse invalidators in `api/invalidate.ts`: `invalidateInventory` for assets/employees/assignments/search, `invalidateAdmin` for members, settings, the activity log and `/meta`. Do not hand-pick keys in a mutation: a write rarely touches only the record you were looking at, and naming subjects is how checking an asset in stopped refreshing the holder's page.
- **The three whole-list pages are server-paged, and nothing here filters an array.** `useAssets`, `useEmployees` and `useMembers` take `{q?, status?, limit, offset}` and **the key carries every parameter** — `['assets', params]`, which still nests under the `['assets']` prefix the invalidators use. Each page keeps `page` in component state, draws `Pagination` at the bottom, reads its footer count and its `pageCount` off `total`, and resets to page one whenever the search or a filter changes. **How many rows a page holds is the reader's, not the screen's**: `usePageSize(surface, fallback)` in `lib/` keeps it in `localStorage` under `inv.rowsPerPage.<surface>` — one key per list, every read and write wrapped so a blocked storage means the fallback rather than a blank page — and `Pagination`'s `rowsPerPage` prop draws the selector. A stored value that is not one of `PAGE_SIZES` falls back, so removing a size cannot strand anybody on it, and a changed size is page one again and a new cache entry, because the key carries `limit`. On Assets that `total` narrows with the status pill — it has to, or the pager offers pages the filter has no rows for — while the pills themselves read `statusCounts`, which ignores the pill. `assetStatusPills` sums those for "All" rather than taking `total`: two numbers, two jobs. `placeholderData: (previous) => previous` is what keeps the table on screen while the next page loads instead of blanking between keystrokes, and `useDebouncedValue` (`lib/`) is what stops a typed word being four requests. `?q=` and `?status=` still live in the URL, so a filtered view is still shareable — only the matching moved.
- **A modal that picks from a list picks from a page of it.** `AssignModal` searches server-side from its own box (`PICKER_PAGE`, and `assignable: true` so a page of twenty is not twenty assets it may not offer). The two employee **dropdowns** — the asset form's assign-on-create and the invite modal's link-to-employee — have no search box, so they ask for `DROPDOWN_LIMIT` (the endpoint's own ceiling) and that is a known limit: give the control a search field before raising it.
...
- **A field error is announced, not only painted.** `Field` gives its error line an id and clones the control it wraps with `aria-invalid` and an `aria-describedby` pointing at it, so every form in the app gained the association at once rather than forty call sites gaining it one by one. `Input` and `Textarea` spread it onto the element; `Dropdown` forwards both onto its trigger the way it forwards `aria-label`. A child that is not a single element keeps the visible message and simply gains no attributes.
...
The bell in `components/app/NotificationsBell.tsx` counts the unread — in its accessible name as well as on the badge — and leads to `/notifications`, `features/notifications/NotificationsPage.tsx`: the whole history, newest first, paged exactly the way the activity log is — numbered pages over `?limit=&offset=`, `total` divided by the page size for how many there are, and the same "Rows per page" selector, whose choice lives under this list's own `inv.rowsPerPage.notifications` key. Both lists grow at the head, so a row arriving while you read page two can shift another onto it; that is the accepted trade-off for pages you can navigate. Every row renders through `renderNotification` from `@inventory/shared` — the audit log's shared-renderer pattern, so the API never sends prose. Reading is the page's own **"Mark all read" button**, deliberately: a page merely glanced at has not been read, so nothing is marked as a side effect of looking, and unread rows keep their weight and accent edge until the button says otherwise. The Settings page's two notification switches decide what the scheduled jobs put there.
...
## Custom fields are the workspace's own vocabulary, and `/custom-fields` is where they are edited

`useCustomFields()` is the definition list — label, the `key` stored values hang off, and the type. `features/custom-fields/CustomFieldsPage.tsx` is the page behind it (flat `/custom-fields`, `requires: 'custom_fields.manage'`, gated in `routes.tsx` like `/workflow`): one row per definition, rename in place, a two-step delete that says it takes the values with it, and the add form at the foot of the card.

It used to be a modal reached from one asset's detail page, which read as editing that asset when it changes the shape of every asset — the same reason statuses and roles are pages. **The asset detail page keeps its custom-values card, read-only**: the values there belong to that asset, and the card header only links a permitted viewer on to the page that owns the definitions.

## API tokens are the one page gated on a role

`/api-tokens` lists the credentials another system uses to call `/api/public/v1`, and it is drawn only for the **admin role** — not for a permission a workspace can grant. That is deliberate and it is the API's rule first: a grantable `tokens.manage` would rebuild the ladder the admin shield closed, because a custom role that can mint an `assets:write` token holds workspace power by proxy, through a credential no session check ever looks at again.

So there is no action to ask about, and three places would otherwise have written `role === 'admin'` in three different words:

- **`isAdmin(role)` in `lib/roles.ts`** is the one sentence, beside `roleInfo` and `leastPrivileged`. `ADMIN_ROLE` stays the only role id this app may name, and this is the only place it is compared.
- **`nav.ts` and `palette.ts` declare it as data**: a `GatedNavItem` (and a palette `ActionDefinition`) carries either `requires: '<action>'` or `adminOnly: true`, never both, and the one filter in each file asks `isAdmin`. `navSectionsFor(permissions, role)` and `paletteGroups({…, role})` therefore take the member's role beside their permissions — the sidebar and `ModalHost` already had the `member`, so nothing new is threaded through the shell.
- **`routes.tsx` gates the route itself** — `isAdmin(member.role) ? <ApiTokensPage /> : <Navigate to="/dashboard" replace />`, exactly the shape the `can(...)` routes use. A hidden nav item hides the door without locking it, here as anywhere else.

**The route is `/api-tokens`, the only client path that begins with those four letters** — and both prefix checks that mean "the API namespace" had to learn the slash before it worked outside in-app navigation: `plugins/static-spa.ts` answered a reload of the page with the JSON 404 envelope, and this workspace's `vite.config.ts` proxied it to the API in dev. Both are `/api/` now, each with a comment. Add another `/api`-adjacent page and the class is already closed; widen either check back and it reopens.

The page itself is a `DataTable` and one modal. **No pager, on purpose**: `GET /api-tokens` has no `limit`/`offset` because a workspace runs a handful of integrations, not a list anybody scrolls. An **expired** token keeps its row with an "Expired" pill until somebody revokes it, so a call that stopped working is diagnosable from the screen; Unlimited and never-used are the design's em dash. Revoking is two steps in the row — "Revoke" in the overflow menu turns the cell into a "Revoke for good" button, the custom fields pattern — because nothing recalls a raw token and every call using it stops at once. Minting ends on `CopyLinkModal` with its own `hint`: the raw value exists in that one response and the database keeps only a hash, the same rule as an invitation link.

## The activity log says what kind of actor wrote each line

Every row carries `actorKind` (`member` | `token` | `system`). A token's name is whatever an admin called it, so "Deploy bot" would read as a colleague — the Actor cell draws an **API pill** beside the name when the kind is `token`, and the toolbar's "Who acted" `Dropdown` narrows the log to one kind. It rides the URL as `?actorKind=`, beside `?type=`, so a filtered log is still a link, and changing either filter goes back to page one. The export link carries both, because the file is what the screen is showing — and the CSV spells the kind out in its own column, since a file cannot draw a pill.

The two numbers behind it serve different masters, which is the rule the pagination PR was written around: `total` obeys **both** filters, because the pager divides it, while `typeCounts` obeys the actor filter and ignores the type one — switching a pill must not move the other pills' numbers.
