...
- **There is no last-admin guard, deliberately.** Nobody may change or remove _their own_ account (409 `self_role_change` / `self_delete`), and every caller is by definition an active admin — so acting on somebody else always leaves at least one. A `last_admin` branch would be unreachable code claiming to protect something.
