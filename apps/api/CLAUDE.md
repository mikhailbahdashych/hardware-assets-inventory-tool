...
- Boot seed (`src/db/seed.ts`) is idempotent and creates two things: the default custom-field defs, and — only on an instance whose `asset_statuses` table is empty — the default workflow. Org settings come from the `/setup` flow. It runs on every start, so it must stay cheap and repeatable. The workflow half is guarded on the table being empty rather than per row, unlike the custom fields: a workspace that has edited its workflow has _deleted_ rows on purpose, and putting them back every boot would be the seed undoing an admin's work.
...
Its **last** step is the curated workflow — the company's own "In imaging" status and a graph with a policy in it, applied through the workflow service so the activity log carries the change. Last is load-bearing: the history above replays under the full mesh a fresh instance is seeded with, and several of those moves are illegal on the curated graph. Anything you add to `demo-data.ts` that moves an asset belongs before it.
