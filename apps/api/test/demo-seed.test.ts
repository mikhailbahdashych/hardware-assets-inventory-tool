...
describe('the demo seed', () => {
  ...
  it('offers one account per role, so permissions can be seen', async () => {
    ...
    const invited = await ctx.db.select().from(members).where(eq(members.status, 'invited')).all();
  ...
  it('leaves a history behind, not just a snapshot', async () => {
    ...
    const closed = await ctx.db
      ...
      .all();
    const returned = (await ctx.db.select().from(assignments).all()).filter(
      (row) => row.returnedAt,
    );
  ...
  it('reads an outcome from the status the holder had at the time', async () => {
    ...
    const jonas = (await ctx.db.select().from(assignments).all()).filter(
  ...
  it('writes an activity log that spans every filter pill', async () => {
    await seeded();
    const rows = await ctx.db.select().from(auditEvents).all();
  ...
  it('curates the workflow instead of leaving the seeded mesh', async () => {
    ...
    const edges = (await ctx.db.select().from(assetStatusTransitions).all()).map(
  ...
  it('replays its history under the seeded mesh, before the curation', async () => {
    ...
    const rewired = (await ctx.db.select().from(auditEvents).all())
      .filter((row) => row.action.startsWith('workflow.'))
      .map((row) => row.at);
    const moves = (await ctx.db.select().from(auditEvents).all())
  ...
  it('curates a fourth role instead of leaving the three it was seeded with', async () => {
    ...
    const rows = await ctx.db.select().from(roles).orderBy(asc(roles.sortOrder)).all();
    ...
    const granted = async (roleId: string) =>
      (await ctx.db.select().from(rolePermissions).where(eq(rolePermissions.roleId, roleId)).all())
        .map((row) => row.action)
        .sort();
  ...
  it('hands that role to the one person whose job it describes', async () => {
    ...
    const holders = await ctx.db.select().from(members).where(eq(members.role, DEMO_ROLE.id)).all();
  ...
  it('refuses a workspace that already has data', async () => {
    ...
    expect((await ctx.db.select().from(employees).all()).length).toBeGreaterThan(0);
  });
  ...
  it('reseeds identically when asked to reset', async () => {
    ...
    const tags = (await ctx.db.select({ tag: assets.assetTag }).from(assets).all()).map(
      (r) => r.tag,
    );
  ...
  it('never leaves a real password in reach', async () => {
    const result = await seeded();
    const rows = await ctx.db.select().from(members).all();
