...
//
// This is the SQLite half of one logical schema; `schema.pg.ts` is the other,
// and `test/schema-parity.test.ts` is what keeps them the same schema. Nothing
// imports this file directly except that test, the drizzle config that
// generates `src/migrations/`, and `schema.ts` — which is what services import
// and what picks a dialect.
