...
| 6   | `feat/06-members-admin`                | Members, invites, the activity log, settings, the danger zone                                         | **Merged** — [#14](https://github.com/mikhailbahdashych/hardware-assets-inventory-tool/pull/14) |
| 7   | `feat/07-dashboard-palette-import`     | Dashboard + widgets, ⌘K palette, CSV import wizard, JSON export                                       | **Merged** — [#15](https://github.com/mikhailbahdashych/hardware-assets-inventory-tool/pull/15) |
| 8   | `feat/08-email-docker-release`         | Mailer + templates, cron jobs, Dockerfile, release workflow, docs                                     | **Merged** — [#16](https://github.com/mikhailbahdashych/hardware-assets-inventory-tool/pull/16) |
| —   | `chore/no-unchecked-indexed-access`    | `noUncheckedIndexedAccess` on, and every index answered honestly                                      | **Merged** — [#18](https://github.com/mikhailbahdashych/hardware-assets-inventory-tool/pull/18) |
| —   | `feat/settings-save-activity-dropdown` | A Save button, a free-form warranty lead time, Activity log as its own page, the `Dropdown` primitive | **Open** — branched from `main`                                                                 |

The eight-PR plan is delivered and merged. Later work branches from `main` one change at a time.
...
- Two admin-only pages, one job each: **`/activity`** is the activity log (counted filter pills, "Load more", a CSV export link) and **`/admin`** is workspace settings, edited as one form and written by a single Save. The old tab URLs still redirect, query string and all.
- **`Dropdown`** is the app's only select — an ARIA select-only combobox with a portalled listbox and full keyboard support. No native `<select>` remains, because the OS drew its own menu inside a design that specifies neither.
...
- **`GET /settings` is admin-only**, so the activity log's "retained for N months" line is written for the only role that can reach the page.
...
- **The Settings page has a Save button, which the design does not draw.** Saving each control as you leave it means a stray keystroke in "Company name" renames the workspace for everybody, with nothing to undo it. The page is one form with one Save: `changedSettings()` diffs the draft against the stored row, and that diff both enables the button and _is_ the payload, so the two cannot disagree.
- **The warranty alert lead time is a number an admin types, not one of 30/60/90.** A dropdown of three was the design's shorthand; nothing in the code branches on the value, it is only compared to a date, so the three choices were telling teams their notice policy was wrong. It is bounded 1–365 in the shared schema and reported in the schema's words when it is not.
- **The activity log is its own page, not a tab inside Admin.** Reading what happened and changing how the workspace behaves are different jobs on different rhythms; as tabs, the log was three clicks deep and the settings shared its heading. Both are still admins-only, and the tab URLs redirect so old links keep working.
