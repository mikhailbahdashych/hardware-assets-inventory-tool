...
**A replacement can also arrive without you asking for one.** `data.aws_ami.al2023` asks for `most_recent = true` and nothing pins what it found last time, so a newer AL2023 published into the region moves the `ami` argument on the next plan — this change, an unrelated one, or no change at all — and a different AMI is `must be replaced`. That is the design working rather than failing: the moving AMI is how this stack picks up OS patches, and a replacement costs the same three minutes as an upgrade, with the Elastic IP moving across and nothing in RDS or S3 noticing. What you should not do is take `must be replaced` here as evidence that you typed something wrong — read the plan and see whether the `ami` line moved too.
...
**What does not follow, and is the whole point of this entry:** **this is not a move, and it is not a destroy-and-recreate either — Terraform simply loses sight of the old stack.** The provider reads its region from this variable, so the next refresh goes looking for the VPC, the instance, the database and the bucket in the _new_ region, finds none of them, and drops all of them out of state as gone. The plan that follows shows creates and nothing else: no destroy line, no prompt, no warning that anything is being left behind. And something is. The old region's stack keeps running, keeps serving whoever still has its address, and keeps billing — while the state file that was the only record of it now describes somewhere else entirely, so there is nothing left to `terraform destroy` it from short of the console.

If you have already changed the variable: put it back, `terraform destroy` in the old region, and stand the new one up afterwards. Do that before any apply — every resource the new region creates is one more thing to unwind.
