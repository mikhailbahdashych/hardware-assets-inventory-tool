...
- **Enums are slugs** (`laptops`, `lost_stolen`) defined in `packages/shared/src/enums.ts` with label and semantic-color maps beside them. The database has **no CHECK constraints** on enums, so adding a value is a code-only change. **Asset statuses are the exception**: they are rows in `asset_statuses` with a transition graph beside them, edited by admins on the Workflow page — `DEFAULT_ASSET_STATUSES` in that file is only what a fresh instance is seeded with. A value the product decides is an enum; a value a workspace decides is a table.
