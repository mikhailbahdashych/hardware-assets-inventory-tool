...
- **Members and invitations** — the accounts that sign in, each holding one of those roles, invited with a copyable link.
...
- **An inbox, not an email server** — warranty alerts, return reminders and hand-over notices land on a bell inside the app, so there is no SMTP to configure and nothing to deliver. Invitations and password resets are links an admin hands over; see [Invitations and recovery](#invitations-and-recovery).
...
Every value has a default. An instance with no configuration at all runs; this table is for the day it has to be reachable from somewhere else, keep its rows in a database server, or put its attachments in a bucket.
...
| `TRUST_PROXY`         | `false`                          | The proxy's own address, CIDR or preset when something terminates TLS in front: `loopback,uniquelocal` for a proxy on the same host, a subnet like `10.0.0.0/16` for a load balancer. Rate limits are keyed on the client address, and without this every request behind a proxy shares one bucket. A hop count is refused at boot — fastify disabled the numeric form because a count cannot verify who connected — and `true` believes the left-most `X-Forwarded-For` entry, which the client writes for itself under an appending proxy (nginx, an AWS load balancer). |
...
Nothing here waits on a mail server, deliberately — a self-hosted tool should not make adding a person depend on deliverability.
...
- Nightly maintenance is the only thing that removes rows nobody asked it to: expired sessions, spent tokens, audit events past the workspace's retention setting, inbox notifications past 90 days, and stored files that no attachment row names.
