...
**Until this project cuts its first release, that image tag does not exist.** Nothing has been pushed to `ghcr.io/mikhailbahdashych/hardware-assets-inventory-tool` yet, so a pull of `:latest` is refused and the `docker run` above has nothing to start. The compose file carries `build: .` beside the image name for exactly this reason: from a checkout of the repo, `docker compose up -d` finds no image to pull, builds one, and runs it — that is the quick start today, and it is one command either way. Tagging a release publishes `:latest`, `:0.2` and `:0.2.0`, for amd64 and arm64, and from that day the two blocks above are literal and pinning a version becomes a decision you can make.
...
| `DATA_DIR`                | `/data` in the image              | What it holds depends on the two switches below: by default the SQLite file **and** the attachments, which makes it the one directory to back up; with `DATABASE_URL` set, the attachments alone; with `S3_BUCKET` set as well, nothing that matters — but it still has to exist and be writable, because the entrypoint probes it at boot. |
...
Two ways to run it locally, and they do the same thing. Full instructions, including Windows and what to do when a port is taken, in [`docs/development.md`](docs/development.md).
